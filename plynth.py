...
# crecentials as environmental varialbes and your Spotify credentials to the
...
def plynth_scan():
    ...
    # Half second delay–not required, but not a bad idea. Recommend increasing this if you're going to use the two-button approach so you have time to get your hand out of the way
    sleep(0.5)
    ...
    try:
        ...
        if len(albums) == 0:
            censoredWords = ['album', 'cover', 'vinyl', 'usa', 'import', 'lp', 'cd', 'soundtrack', 'german import']
            word_list = album_guess.split()
            album_guess = ' '.join([i for i in word_list if i not in censoredWords])
            albums = spotify_search(album_guess)
        
        # If still no albums are found, removes the last word anad retries
        while len(albums) == 0:
...
# Plynth waits for a button press–scan, stop or off. 
# All are the same button, set at different duractions
scan_button = Button(5)
stop_button = Button(5, hold_time=2)
scan_button.when_pressed = plynth_scan
stop_button.when_pressed = plynth_stop
...
# on/off button for the Raspberry Pi
